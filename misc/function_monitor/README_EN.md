...
3. Import the data into [MindStudio Insight](https://gitcode.com/Ascend/msinsight/blob/26.1.0/docs/en/user_guide/overview.md) for visualization to analyze the relationship between function execution duration and CPU PMU metrics.
...
1. Build and install the `libkperf` Python wheel package. For details, see [libkperf Build Guide](https://gitcode.com/openeuler/libkperf#%E7%BC%96%E8%AF%91).
...
You can run the `perf list` command to view all supported perf events. For details, see [perf event official documentation](https://man7.org/linux/man-pages/man1/perf-list.1.html). You can select perf events as needed.
