...
3. 通过 MindStudio 提供的 [ftrace_tools](https://gitcode.com/Ascend/msinsight/tree/26.2.0/scripts/ftrace_tools) 工具，完成了 CPU 调度事件的采集工作，采集结果如下：
...
4. 隔离干扰进程：观测到大量 CSD 线程存在频繁的上下文切换，经确认属于 dpc（分布式文件共享系统）子线程。使用 MindStudio 提供的[自动化绑核](https://gitcode.com/Ascend/msprof/tree/26.2.0/misc/host_analyzer)工具，将 dpc 进程与训练进程分别绑定至不同的 CPU 核区间，实现资源隔离，避免相互抢占干扰。
