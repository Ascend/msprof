...
class FileNameManagerConstant:
    ...
    GE_MODEL_TIME_TRAINING_FILE_PATTERN = r"^Framework.host.model_time_info_(\d+)_(\d+)\.(\d+)\.slice_\d+"
    ...
    HWTS_TRAINING_FILE_PATTERN = r"hwts\.log\.data\.\d+\.(dev|host)\.profiler_default_tag\.(\d+)\.slice_\d+"
    ...
    L2_CACHE_TRAINING_FILE_PATTERN = r"l2_cache\.data\.\d+\.(dev|host)\.profiler_default_tag\.(\d+)\.slice_\d+"
    ...
    NIC_TRAINING_FILE_PATTERN = r"^nic\.data\.\d+\.(dev|host)\.profiler_default_tag\.(\d+)\.slice_\d+"

    PCIE_FILE_PATTERN = r"^pcie\.data\.(\d+)\.slice_\d+"
    PCIE_TRAINING_FILE_PATTERN = r"^pcie\.data\.\d+\.(dev|host)\.profiler_default_tag\.(\d+)\.slice_\d+"
    ...
    PID_CPU_USAGE_TRAINING_FILE_PATTERN = r"(\d+)-CpuUsage\.data\.dev\.profiler_default_tag\.(\d+)\.slice_\d+"

    ROCE_FILE_PATTERN = r"^roce\.data\.(\d+)\.slice_\d+"
    ROCE_TRAINING_FILE_PATTERN = r"roce\.data\.(\d+)\.(dev|host)\.profiler_default_tag\.(\d+)\.slice_\d+"
    ...
    TASK_TRACK_V2_COMPACT_FILE_PATTERN = r"^(aging|unaging)\.compact\.task_track_v2\.slice_\d+"
    ...
    SYS_MEM_TRAINING_FILE_PATTERN = r"^Memory\.data\.(dev|host)\.profiler_default_tag\.(\d+)\.slice_\d+"
    ...
    SYS_CPU_USAGE_TRAINING_FILE_PATTERN = r"^SystemCpuUsage\.data\.dev\.profiler_default_tag\.(\d+)\.slice_\d+"

    TS_CPU_FILE_PATTERN = r"^tscpu\.data\.(\d+)\.slice_\d+"
    TS_CPU_INFER_FILE_PATTERN = r"^tscpu\.data\.\d+\.(\d+)\.\d+"
    TS_CPU_TRAINING_FILE_PATTERN = r"^tscpu\.data\.(\d+)\.(dev|host)\.profiler_default_tag\.(\d+)\.slice_\d+"

    TS_TRACK_FILE_PATTERN = r"^ts_track\.data\.(\d+)\.slice_\d+"
    TS_TRACK_INFER_FILE_PATTERN = r"ts_track\.data\.\d+\.(\d+)\.\d+"
    TS_TRACK_TRAINING_FILE_PATTERN = r"ts_track\.data\.\d+\.dev\.profiler_default_tag\.(\d+)\.slice_\d+"
    ...
    CAPTURE_STREAM_INFO_V2_PATTERN = r"^(unaging|aging)\.compact\.capture_stream_info_v2\.slice_\d+"
...
def get_ai_core_compiles() -> tuple:
    ...
    """
    ai_core_compiles = AiCorePattern(
        re.compile(FileNameManagerConstant.AI_CORE_FILE_PATTERN),
...
def get_ai_cpu_compiles() -> tuple:
    ...
    """
    return (
        re.compile(FileNameManagerConstant.AI_CPU_FILE_PATTERN),
        re.compile(FileNameManagerConstant.AI_CPU_INFER_FILE_PATTERN),
        re.compile(FileNameManagerConstant.AI_CPU_TRAINING_FILE_PATTERN),
    )
...
def get_ctrl_cpu_compiles() -> tuple:
    ...
    """
    return (
        re.compile(FileNameManagerConstant.CTRL_CPU_FILE_PATTERN),
        re.compile(FileNameManagerConstant.CTRL_CPU_INFER_FILE_PATTERN),
        re.compile(FileNameManagerConstant.CTRL_CPU_TRAINING_FILE_PATTERN),
    )
...
def get_ddr_compiles() -> tuple:
    ...
    """
    return (
        re.compile(FileNameManagerConstant.DDR_FILE_PATTERN),
        re.compile(FileNameManagerConstant.DDR_INFER_FILE_PATTERN),
        re.compile(FileNameManagerConstant.DDR_TRAINING_FILE_PATTERN),
    )
...
def get_dvpp_compiles() -> tuple:
    ...
    """
    return (
        re.compile(FileNameManagerConstant.DVPP_FILE_PATTERN),
        re.compile(FileNameManagerConstant.DVPP_INFER_FILE_PATTERN),
        re.compile(FileNameManagerConstant.DVPP_TRAINING_FILE_PATTERN),
    )
...
def get_ge_tensor_info_compiles() -> tuple:
    ...
    """
    return re.compile(FileNameManagerConstant.GE_TENSOR_INFO_PATTERN), re.compile(
        FileNameManagerConstant.GE_TENSOR_INFO_SINGLE_PATTERN
    )
...
def get_ge_model_load_compiles() -> tuple:
    ...
    """
    return (
        re.compile(FileNameManagerConstant.GE_MODEL_LOAD_FILE_PATTERN),
        re.compile(FileNameManagerConstant.GE_MODEL_LOAD_INFER_FILE_PATTERN),
        re.compile(FileNameManagerConstant.GE_MODEL_LOAD_TRAINING_FILE_PATTERN),
    )
...
def get_ge_model_time_compiles() -> tuple:
    ...
    """
    return (
        re.compile(FileNameManagerConstant.GE_MODEL_TIME_FILE_PATTERN),
        re.compile(FileNameManagerConstant.GE_MODEL_TIME_INFER_FILE_PATTERN),
        re.compile(FileNameManagerConstant.GE_MODEL_TIME_TRAINING_FILE_PATTERN),
    )
...
def get_hwts_compiles() -> tuple:
    ...
    """
    return (
        re.compile(FileNameManagerConstant.HWTS_FILE_PATTERN),
        re.compile(FileNameManagerConstant.HWTS_INFER_FILE_PATTERN),
        re.compile(FileNameManagerConstant.HWTS_TRAINING_FILE_PATTERN),
    )
...
def get_l2_cache_compiles() -> tuple:
    ...
    """
    return (
        re.compile(FileNameManagerConstant.L2_CACHE_FILE_PATTERN),
        re.compile(FileNameManagerConstant.L2_CACHE_INFER_FILE_PATTERN),
        re.compile(FileNameManagerConstant.L2_CACHE_TRAINING_FILE_PATTERN),
    )
...
def get_llc_compiles() -> tuple:
    ...
    """
    return (
        re.compile(FileNameManagerConstant.LLC_FILE_PATTERN),
        re.compile(FileNameManagerConstant.LLC_INFER_FILE_PATTERN),
        re.compile(FileNameManagerConstant.LLC_TRAINING_FILE_PATTERN),
    )
...
def get_runtime_task_track_compiles() -> tuple:
    ...
    """
    return (
        re.compile(FileNameManagerConstant.RUNTIME_TASK_TRACK_FILE_PATTERN),
        re.compile(FileNameManagerConstant.RUNTIME_TASK_TRACK_INFER_FILE_PATTERN),
        re.compile(FileNameManagerConstant.RUNTIME_TASK_TRACK_TRAINING_FILE_PATTERN),
    )
...
def get_ts_cpu_compiles() -> tuple:
    ...
    """
    return (
        re.compile(FileNameManagerConstant.TS_CPU_FILE_PATTERN),
        re.compile(FileNameManagerConstant.TS_CPU_INFER_FILE_PATTERN),
        re.compile(FileNameManagerConstant.TS_CPU_TRAINING_FILE_PATTERN),
    )
...
def get_ts_track_compiles() -> tuple:
    ...
    """
    return (
        re.compile(FileNameManagerConstant.TS_TRACK_FILE_PATTERN),
        re.compile(FileNameManagerConstant.TS_TRACK_INFER_FILE_PATTERN),
        re.compile(FileNameManagerConstant.TS_TRACK_TRAINING_FILE_PATTERN),
    )
...
def get_biu_compiles() -> tuple:
    ...
    """
    return (
        re.compile(FileNameManagerConstant.BIU_PERF_FILE_PATTERN),
...
def get_npu_mem_compiles() -> tuple:
    ...
    """
    return (
        re.compile(FileNameManagerConstant.NPU_MEM_FILE_PATTERN),
...
def get_lpm_info_compiles() -> tuple:
    ...
    """
    return (
        re.compile(FileNameManagerConstant.LPM_INFO_FILE_PATTERN),
...
def get_task_track_compact_compiles() -> tuple:
    """
    get runtime task track regex compiles (V1 + V2)
    """
    return (
        re.compile(FileNameManagerConstant.TASK_TRACK_COMPACT_FILE_PATTERN),
        re.compile(FileNameManagerConstant.TASK_TRACK_V2_COMPACT_FILE_PATTERN),
    )
...
def get_memcpy_info_compact_compiles() -> tuple:
    ...
    """
    return (re.compile(FileNameManagerConstant.MEMCPY_INFO_COMPACT_FILE_PATTERN),)
...
def get_capture_stream_info_compiles() -> tuple:
    """
    get capture stream info regex compiles (V1 + V2)
    :return: capture stream info data regex
    """
    return (
        re.compile(FileNameManagerConstant.CAPTURE_STREAM_INFO_PATTERN),
        re.compile(FileNameManagerConstant.CAPTURE_STREAM_INFO_V2_PATTERN),
    )
...
def get_runtime_op_info_compiles() -> tuple:
    ...
    """
    return (
        re.compile(FileNameManagerConstant.RUNTIME_OP_INFO_PATTERN),
...
def get_dpu_track_compact_compiles() -> tuple:
    ...
    """
    return (re.compile(FileNameManagerConstant.DPU_TASK_TRACK_FILE_PATTERN),)
...
def get_dpu_hccl_track_compact_compiles() -> tuple:
    ...
    get runtime task track regex compiles
    """
    return (re.compile(FileNameManagerConstant.DPU_HCCL_TRACK_FILE_PATTERN),)
