...
class HcclViewModel(ViewModel):
    ...
    def get_hccl_task_data(self):
        ...
        sql = (
            ...
            "a.local_rank as local_rank, a.remote_rank as remote_rank, "
            "a.transport_type as transport_type, a.size as size, a.data_type as data_type, "
            "a.link_type as link_type, a.thread_id as thread_id, a.op_id as op_id, "
            "a.context_id as context_id, a.notify_id as notify_id, a.batch_id as batch_id, "
            "a.rdma_type as rdma_type, b.duration as duration, a.rank_size as rank_size from {0} as a "
    ...
    def get_hccl_ops(self, model_id: int, index_id: int):
        ...
        sql = (
            "SELECT model_id, index_id, op_name, task_type, op_type, connection_id, thread_id, "  # nosec B608
            "relay, retry, data_type, alg_type, count, group_name, kfc_connection_ids from {0} "
    ...
    def get_hccl_op_info_from_table(self: any, table_name: str = DBNameConstant.TABLE_HCCL_OP_SINGLE_DEVICE):
        ...
        if not DBManager.judge_table_exist(self.cur, table_name):
            return []
        sql = f"select model_id, op_name, relay, retry, data_type, alg_type, count, group_name, connection_id, rank_size, start, end from {table_name}"  # nosec B608
        return DBManager.fetch_all_data(self.cur, sql, dto_class=HcclOps)

    def get_hccl_op_time_section(self: any, table_name: str = DBNameConstant.TABLE_HCCL_OP_SINGLE_DEVICE):
        if not DBManager.judge_table_exist(self.cur, table_name):
            return []
        sql = f'select start as start_time, end as end_time from {table_name}'
