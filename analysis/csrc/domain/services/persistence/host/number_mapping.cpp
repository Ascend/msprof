...
const std::unordered_map<uint32_t, std::string> geTaskTypeMap{
    {0, "AI_CORE"}, {1, "AI_CPU"},    {2, "AI_VECTOR_CORE"}, {3, "WRITE_BACK"}, {4, "MIX_AIC"},
    {5, "MIX_AIV"}, {6, "FFTS_PLUS"}, {7, "DSA_SQE"},        {8, "DVPP"},       {9, "COMMUNICATION"},
    {10, "FUSION"}, {11, "INVALID"},  {256, "HCCL_AI_CPU"}};
...
const std::unordered_map<uint32_t, std::string> hcclLinkTypeMap{{0, "ON_CHIP"},
                                                                {1, "HCCS"},
                                                                {2, "PCIE"},
                                                                {3, "ROCE"},
                                                                {4, "SIO"},
...
const std::unordered_map<uint32_t, std::string> hcclTransportTypeMap{
    {0, "SDMA"}, {1, "RDMA"}, {2, "LOCAL"}, {3, "UB"}, {4, "ROCE"}, {255, "RESERVED"}, {4294967295, "INVALID_TYPE"},
};
const std::unordered_map<uint32_t, std::string> hcclRdmaTypeMap{
    {0, "RDMA_SEND_NOTIFY"}, {1, "RDMA_SEND_PAYLOAD"}, {255, "RESERVED"}, {4294967295, "INVALID_TYPE"}};
const std::unordered_map<uint32_t, std::string> hcclOpTypeMap{
    {0, "SUM"}, {1, "MUL"}, {2, "MAX"}, {3, "MIN"}, {255, "RESERVED"}, {4294967295, "INVALID_TYPE"}};
const std::unordered_map<uint32_t, std::string> levelMap{{30000, "pytorch"},      {25000, "pta"},   {20500, "msproftx"},
                                                         {20000, "acl"},          {15000, "model"}, {10000, "node"},
                                                         {5500, "communication"}, {5000, "runtime"}};
const std::unordered_map<uint32_t, std::string> aclApiTagMap{
    {1, "ACL_OP"},      {2, "ACL_MODEL"}, {3, "ACL_RTS"},  {4, "ACL_OTHERS"}, {5, "ACL_NN"},
    {6, "ACL_ASCENDC"}, {7, "HOST_HCCL"}, {9, "ACL_DVPP"}, {10, "ACL_GRAPH"}, {11, "ACL_ATB"},

};
std::vector<std::unordered_map<uint32_t, std::string>> allMaps{
    geDataTypeMap,        geFormatMap,     geTaskTypeMap, hcclDataTypeMap, hcclLinkTypeMap,
    hcclTransportTypeMap, hcclRdmaTypeMap, hcclOpTypeMap, levelMap,        aclApiTagMap};
