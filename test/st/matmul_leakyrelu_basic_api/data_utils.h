...
{
    struct stat sBuf;
    int fileStatus = stat(filePath.c_str(), &sBuf);
    if (fileStatus == -1) {
    ...
    size_t size = static_cast<size_t>(buf->pubseekoff(0, std::ios::end, std::ios::in));
    if (size == 0) {
    ...
    if (static_cast<size_t>(buf->sgetn(static_cast<char*>(buffer), size)) != size) {
