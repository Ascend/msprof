...
class TestAscendMsprofAppSampleBasedResourceConflictRatio(BaseAscendMsprofChecker):
    ...
    AI_VECTOR_CORE_UTILIZATION_SPEC = {
        ...
        "headers": ["vec_bankgroup_cflt_ratio", "vec_bank_cflt_ratio", "vec_resc_cflt_ratio"],
        "non_negative_columns": ["vec_bankgroup_cflt_ratio", "vec_bank_cflt_ratio", "vec_resc_cflt_ratio"],
    }
...
def test_ascend_msprof_app_sample_based_resource_conflict_ratio(prof_path):
    TestAscendMsprofAppSampleBasedResourceConflictRatio(prof_path).check_msprof_file()
