...
def test_ascend_msprof_acl_graph_and_triton_inductor(prof_path):
    ...
    matched = df[df["Op Name"].str.lower().str.startswith("triton", na=False)]
    if matched.empty:
        raise AssertionError("No rows with Op Name starting with 'triton' found in op_summary")

    valid = matched[matched["Model ID"] != DEFAULT_MODEL_ID]
    if valid.empty:
        raise AssertionError(f"All triton rows have invalid Model ID {DEFAULT_MODEL_ID}")


if __name__ == '__main__':
    path = sys.argv[1]
    test_ascend_msprof_acl_graph_and_triton_inductor(path)
