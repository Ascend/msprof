...
class TestAscendMsprofAppSampleBasedMemory(BaseAscendMsprofChecker):
    EXPECT_DB_TABLES = [
    ...
    AI_VECTOR_CORE_UTILIZATION_SPEC = {
        ...
        "headers": ["ub_read_bw(GB/s)", "ub_write_bw(GB/s)", "l1_read_bw(GB/s)", "l1_write_bw(GB/s)", "l2_read_bw(GB/s)", "l2_write_bw(GB/s)", "main_mem_read_bw(GB/s)", "main_mem_write_bw(GB/s)"],
        "non_negative_columns": ["ub_read_bw(GB/s)", "ub_write_bw(GB/s)", "l1_read_bw(GB/s)", "l1_write_bw(GB/s)", "l2_read_bw(GB/s)", "l2_write_bw(GB/s)", "main_mem_read_bw(GB/s)", "main_mem_write_bw(GB/s)"],
    }
...
def test_ascend_msprof_app_sample_based_memory(prof_path):
    TestAscendMsprofAppSampleBasedMemory(prof_path).check_msprof_file()


if __name__ == "__main__":
    path = sys.argv[1]
    test_ascend_msprof_app_sample_based_memory(path)
