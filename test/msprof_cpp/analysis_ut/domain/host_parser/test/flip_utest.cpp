...
protected:
    ...
    static void SetTaskTrack(std::shared_ptr<ParserCompactInfo> &taskTrack,
                             const uint64_t &timeStamp, const ParserRuntimeTrack &runtimeTrack)
    {
        taskTrack->timeStamp = timeStamp;
        taskTrack->data.runtimeTrack = runtimeTrack;
    }

    static void CheckBatchId(const std::vector<std::shared_ptr<ParserCompactInfo>> &taskTrack, uint16_t dataNum,
...
{
    std::vector<std::shared_ptr<ParserCompactInfo>> taskTrack {};
...
TEST_F(FlipUTest, TestComputeBatchIdShoulSetBatchId0WhenFlipDataIsEmpty)
{
    ...
    auto taskTrack = CreateData<ParserCompactInfo>(dataNum);
...
TEST_F(FlipUTest, TestComputeBatchIdShouldSetBatchIdWithEachStreamWhenFlipSepTaskData)
{
    ...
    auto taskTrack = CreateData<ParserCompactInfo>(compactInfoDataNum);
